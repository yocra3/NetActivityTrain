...
#'#################################################################################
#'#################################################################################
#'  Train a sparsely connected neural network
#'#################################################################################
#'#################################################################################
...
## Load genes
with open(input_genes,'r') as file:
    genes = file.read()
genes = genes.split('\n')[0:-1]
...
gene_map_filt = gene_map[gene_map['Gene'].isin(genes)]
pathway_count = gene_map_filt['GeneSetID'].value_counts()

gene_dict = dict(zip(genes, range(len(genes))))
gene_map_filt['idx'] = [gene_dict[gene] for gene in gene_map_filt['Gene']]

## Create list of inputs - pathway
pathway_dict = {}
for g, idx in zip(gene_map_filt['GeneSetID'].values, gene_map_filt['idx'].values):
...
df.to_csv(name + '_pathways_names.txt',  sep = "\t", index = False)
